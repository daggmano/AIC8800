...
static int rwnx_set_mac_address(struct net_device *dev, void *addr)
{
    ...
    if (!sa || sa->sa_family != ARPHRD_ETHER)
        return -EINVAL;
    if (!is_valid_ether_addr(sa->sa_data))
        return -EADDRNOTAVAIL;

    ret = eth_prepare_mac_addr_change(dev, sa);
    if (ret)
        return ret;

    eth_commit_mac_addr_change(dev, sa);
                                               ...
                                               struct vif_params *params)
{
    ...
    if (type == NL80211_IFTYPE_AP_VLAN) {
        ...
        ether_addr_copy(vif->wdev.address, ndev->dev_addr);
    } else {
        ...
        mac_addr[0] |= 0x02;              /* set locally-administered */
